...
import { selectCurrentWeek, selectCurrentPhase, selectMilestoneProgress } from './tracker.js';
export function buildTaskContext(state, subtask, milestone) {
    ...
    if (subtask.notes)
        sections.push(`**Notes:** ${subtask.notes}`);
    if (subtask.blocked_reason)
        sections.push(`**Blocked Reason:** ${subtask.blocked_reason}`);
    sections.push('');
    ...
    const revisions = state.agent_log.filter(e => e.target_id === subtask.id && e.action === 'revision_requested');
...
export function buildMilestoneOverview(milestone, state) {
    ...
    if (milestone.actual_start)
        sections.push(`- **Actual Start:** ${milestone.actual_start}`);
    if (milestone.actual_end)
        sections.push(`- **Actual End:** ${milestone.actual_end}`);
    if (milestone.drift_days !== 0) {
        sections.push(`- **Drift:** ${milestone.drift_days > 0 ? '+' : ''}${milestone.drift_days} days`);
    }
