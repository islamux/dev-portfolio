import { readTracker, writeTracker, findTask, touchAgent, autoUnblockDependents, generateTaskId, generateLogId, todayDateString, selectMilestoneProgress, logger, } from './tracker.js';
...
import { GetTaskContextSchema, GetTaskHistorySchema, GetMilestoneOverviewSchema, ListTasksSchema, GetActivityFeedSchema, StartTaskSchema, CompleteTaskSchema, ApproveTaskSchema, RejectTaskSchema, BlockTaskSchema, UnblockTaskSchema, UpdateTaskSchema, LogActionSchema, EnrichTaskSchema, CreateMilestoneSchema, AddMilestoneTaskSchema, SetMilestoneDatesSchema, UpdateDriftSchema, AddMilestoneNoteSchema, RegisterAgentSchema, } from './schemas.js';
function snapshotEntity(entity) {
    return JSON.stringify(entity);
}
function logAndWrite(state, toolName, before, after) {
    appendUndoEntry({
...
function makeLogEntry(agentId, action, targetType, targetId, description, tags = []) {
    return {
        id: generateLogId(),
        agent_id: agentId,
        action,
...
// ─── READ TOOLS ───────────────────────────────────────────────
function handleGetTaskContext(args) {
    ...
    if (!found)
        return err(`Task '${v.data.task_id}' not found in any milestone`);
    return ok(buildTaskContext(state, found.subtask, found.milestone));
}
function handleGetTaskSummary(args) {
    const v = validate(GetTaskContextSchema, args);
    if (v.error)
        return err(v.error);
    const state = readTracker();
...
function handleGetMilestoneOverview(args) {
    ...
    if (!milestone)
        return err(`Milestone '${v.data.milestone_id}' not found`);
...
function handleListTasks(args) {
    ...
    for (const m of state.milestones) {
        if (milestone_id && m.id !== milestone_id)
            continue;
    ...
    if (tasks.length === 0)
        return ok('No tasks found matching filters.');
...
function handleGetTaskHistory(args) {
...
function handleGetActivityFeed(args) {
    const v = validate(GetActivityFeedSchema, args);
    if (v.error)
        return err(v.error);
    const state = readTracker();
    let entries = [...state.agent_log].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    if (v.data.agent_id) {
        entries = entries.filter(e => e.agent_id === v.data.agent_id);
    }
    entries = entries.slice(0, v.data.limit);
    if (entries.length === 0)
        return ok('No activity recorded.');
    const lines = entries.map(e => {
        const tags = e.tags.length > 0 ? ` [${e.tags.join(', ')}]` : '';
...
function handleApproveTask(args) {
    ...
    if (task.status !== 'review')
        return err(`Task '${task.id}' is in status '${task.status}', expected 'review'`);
    const before = snapshotEntity(task);
    task.status = 'done';
    task.done = true;
    ...
    if (allDone)
        milestone.actual_end = todayDateString();
...
function handleRejectTask(args) {
    ...
    const before = snapshotEntity(task);
    task.status = 'in_progress';
    const revisionCount = state.agent_log.filter(e => e.target_id === task.id && e.action === 'revision_requested').length + 1;
...
function handleUnblockTask(args) {
    ...
    if (task.status !== 'blocked')
        return err(`Task '${task.id}' is in status '${task.status}', expected 'blocked'`);
    const before = snapshotEntity(task);
...
function handleUpdateTask(args) {
    ...
    const before = snapshotEntity(task);
    const changes = [];
    if (v.data.priority !== undefined) {
        changes.push(`priority: ${task.priority} → ${v.data.priority}`);
        task.priority = v.data.priority;
    }
    if (v.data.assignee !== undefined) {
        changes.push(`assignee: ${task.assignee} → ${v.data.assignee || 'unassigned'}`);
        task.assignee = v.data.assignee || null;
    }
    if (v.data.execution_mode !== undefined) {
        changes.push(`execution_mode: ${task.execution_mode} → ${v.data.execution_mode}`);
        task.execution_mode = v.data.execution_mode;
    }
    if (v.data.notes !== undefined) {
        changes.push('notes updated');
        task.notes = v.data.notes;
    }
    if (changes.length === 0)
        return ok('No changes specified.');
...
function handleEnrichTask(args) {
    ...
    const before = snapshotEntity(task);
    const changes = [];
    if (v.data.prompt !== undefined) {
        task.prompt = v.data.prompt;
        changes.push('prompt');
    }
    if (v.data.builder_prompt !== undefined) {
        task.builder_prompt = v.data.builder_prompt;
        changes.push('builder_prompt');
    }
    if (v.data.acceptance_criteria !== undefined) {
        task.acceptance_criteria = v.data.acceptance_criteria;
        changes.push('acceptance_criteria');
    }
    if (v.data.constraints !== undefined) {
        task.constraints = v.data.constraints;
        changes.push('constraints');
    }
    if (v.data.context_files !== undefined) {
        task.context_files = v.data.context_files;
        changes.push('context_files');
    }
    if (v.data.reference_docs !== undefined) {
        task.reference_docs = v.data.reference_docs;
        changes.push('reference_docs');
    }
    if (changes.length === 0)
        return ok('No enrichment fields specified.');
...
function handleAddMilestoneNote(args) {
...
function handleSetMilestoneDates(args) {
    ...
    if (!milestone)
        return err(`Milestone '${v.data.milestone_id}' not found`);
    const changes = [];
    if (v.data.actual_start !== undefined) {
        milestone.actual_start = v.data.actual_start;
        changes.push(`actual_start: ${v.data.actual_start}`);
    }
    if (v.data.actual_end !== undefined) {
        milestone.actual_end = v.data.actual_end;
        changes.push(`actual_end: ${v.data.actual_end}`);
    }
...
function handleUpdateDrift(args) {
...
function handleCreateMilestone(args) {
    const v = validate(CreateMilestoneSchema, args);
    if (v.error)
        return err(v.error);
    const state = readTracker();
    if (state.milestones.find(m => m.id === v.data.id))
        return err(`Milestone '${v.data.id}' already exists`);
    const milestone = {
        id: v.data.id,
        title: v.data.title,
...
function handleAddMilestoneTask(args) {
...
function handleRegisterAgent(args) {
    ...
    if (existing) {
        ...
        if (v.data.parent_id !== undefined)
            existing.parent_id = v.data.parent_id;
    }
    else {
        const agent = {
            id: v.data.agent_id,
        ...
        if (v.data.parent_id !== undefined)
            agent.parent_id = v.data.parent_id;
